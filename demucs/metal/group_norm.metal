...
//
// All reductions accumulate in FP32; FP16 only crosses the device-memory
// boundary at load and store, avoiding the implicit cast traffic that makes
// PyTorch's stock FP16 GroupNorm slow on MPS.
...
kernel void partial_reduce_fp16(
    device const half*  in_           [[buffer(0)]],
...
kernel void apply_norm_fp16(
    ...
    device const half*  weight       [[buffer(3)]],    // (C,)
    device const half*  bias         [[buffer(4)]],    // (C,)
